  ...
  <div class="wrap">
    <ul class="list">
      ...
      <li>
        ...
        <button class="yzm" :class="identifyBtn?'yzmdis':''" :disabled="identifyBtn" @click="getIdentify">{{cont}}</button>
      </li>
    </ul>
    <button :class="disabledFn?'':'disable'" :disabled="!disabledFn" @click="save" id="btn">验证后绑定新手机</button>
    ...
    methods: {
      ...
      async save() {
        ...
        if (result.res_code === 1){
          wx.showToast({
            title: result.message, //提示的内容,
            icon: 'none', //图标,
          });
        }
        // 更新trackId
        this.updateState({
          tranckId: result.result.trackId
        })
        // 判断是否有parentId
        if (!result.result.parentId){
          wx.redirectTo({ url: '/pages/referrer/main' });
        }
      }
    }
  };
